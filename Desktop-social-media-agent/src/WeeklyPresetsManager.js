...
export default function WeeklyPresetsManager({ presets, setPresets }) {
  // State for presets
  ...
  // Update day topic
  const updateDayTopic = (day, topic) => {
    setCurrentPreset(prev => ({
      ...prev,
      schedule: {
        ...prev.schedule,
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                      currentPreset.schedule[day].enabled ? 'bg-amber-600' : 'bg-gray-200'
                    }`}
                    aria-label={`Toggle ${day} schedule`}
                  >
                    {currentPreset.schedule[day].enabled ? (
                ...
                {currentPreset.schedule[day].enabled && (
                  <>
                    <div className="mb-2">
                      ...
                      <select
                        value={currentPreset.schedule[day].topic}
                    ...
                    <div>
                      ...
                      <input
                        type="time"
                        value={currentPreset.schedule[day].time}
              ...
              <div key={platform} className="bg-amber-50 p-4 rounded-lg shadow">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                      currentPreset.platforms[platform] ? 'bg-amber-600' : 'bg-gray-200'
                    }`}
                    aria-label={`Toggle ${platform} platform`}
                  >
                    {currentPreset.platforms[platform] ? (
                      <Check className="w-4 h-4 text-white" />
                    ) : (
                      <Plus className="w-4 h-4 text-amber-600" />
                    )}
